...
// Lazyload for posts
if ( handy_get_option('lazyload_on_blog')==='on' ) {
	$classes[] = 'lazyload';
} ?>
...
<article id="post-<?php the_ID(); ?>" <?php post_class($classes); ?> itemscope="itemscope" itemtype="http://schema.org/Article" <?php if ( !is_single() ) { echo ' data-expand="-100"'; } ?>><!-- Article ID-<?php the_ID(); ?> -->
	...
	<?php
		if ( is_sticky() && is_home() && ! is_paged() ) {
			printf( '<span class="sticky-post">%s</span>', __( 'Featured', 'handystore' ) );
		}
	?>
		...
		<?php
			...
			elseif ( is_search() ) : // Search Results
				...
				<h1 class="entry-title search-title" itemprop="headline">
					<a href="<?php esc_url(the_permalink()); ?>" title="<?php echo esc_attr( sprintf( __( 'Click to read more about %s', 'handystore' ), the_title_attribute( 'echo=0' ) ) ); ?>" rel="bookmark" itemprop="url"><?php echo $title; ?></a>
				</h1>
			<?php else :
				$title = get_the_title();
				if ( empty($title) || $title = '' ) { ?>
					<h1 class="entry-title" itemprop="headline">
						...
						<a href="<?php esc_url(the_permalink()); ?>" title="<?php _e( 'Click here to read more', 'handystore' ); ?>" rel="bookmark" itemprop="url"><?php _e( 'Click here to read more', 'handystore' ); ?></a>
					</h1>
				<?php } else {
	...
	<div class="entry-meta">
		...
		<?php edit_post_link( __( 'Edit', 'handystore' ), '<span class="edit-link">', '</span>' ); ?>
	</div>
	...
	<?php else : ?>
		<div class="entry-content" itemprop="articleBody"><!-- Content -->
			...
			<?php wp_link_pages( array(
				'before'      => '<div class="page-links"><span class="page-links-title">' . __( 'Pages:', 'handystore' ) . '</span>',
				'after'       => '</div>',
				'link_before' => '<span>',
				'link_after'  => '</span>',
				'pagelink'    => '%',
