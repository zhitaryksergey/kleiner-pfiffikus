...
function pt_layouts_metabox() {
	$screen = 'page';
    add_meta_box( 'layout_id', __( 'Layout Settings', 'handystore' ), 'pt_layout_metabox_contents', $screen, 'side');
}
...
function pt_layout_metabox_contents($post) {
  ...
  $current_layout = pt_get_post_layout( $post->ID ); ?>
	<div class="post-layout">
		<p><?php esc_html__( 'Specify a custom page layout for this content.', 'handystore' ) ?></p>
		<input type="radio" name="pt_post_layout" id="post-layout-default" value="default" <?php checked( $current_layout, "default" )?> />
		<label for="post_layout_default"><?php esc_html_e( 'Default Set in Handy Store Theme Options','handystore') ?></label><br /><br />
...
function pt_get_default_layouts() {
	...
	if (isset( $layouts[0] )){
		...
		__('Specify some layouts first!', 'handystore');};
};

function pt_get_term_layout( $term_id ) {
	/* Get the post layout. */
	$layout = get_term_meta( $term_id, '-pt-layout', true );
