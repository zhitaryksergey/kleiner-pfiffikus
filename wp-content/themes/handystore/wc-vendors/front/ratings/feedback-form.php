...
<h2><?php _e('Rate your experience', 'handystore'); ?></h2>

<p>
<?php printf( __( 'Order #<mark class="order-number">%s</mark> was placed on <mark class="order-date">%s</mark> and is currently <mark class="order-status">%s</mark>.', 'handystore' ), $order->get_order_number(), date_i18n( get_option( 'date_format' ), strtotime( $order->order_date ) ), wc_get_order_status_name( $order->get_status() ) ); ?>
</p>
...
foreach ( $products as $product ) {
	...
	if ( is_string( get_post_status( $product['product_id'] ) ) ) {
		echo '<a href="' . get_permalink( $product_id ) . '">' . $product['name'] . '</a>'. __( ' from ', 'handystore') .$shop_name.'</br>';
	} else {
		echo $product['name'] . __( ' from ', 'handystore') .$shop_name.'</br>';
	}
	...
	<p></p>
	<p>
		<input type="text" name="wcv-feedback[<?php echo $fn; ?>][rating_title]" style="width:60%" value="<?php echo $rating_title; ?>" placeholder="<?php _e( 'Title', 'handystore'); ?>" />
	</p>
	<p>
		<textarea name="wcv-feedback[<?php echo $fn; ?>][comments]" style="width:60%" placeholder="<?php _e( '(e.g. delivery experience, item as described, quality of customer service)', 'handystore'); ?>"><?php echo $comments; ?></textarea>
	</p>
...
<p><input type="submit" value="<?php _e('Submit Feedback', 'handystore'); ?>"></p>
