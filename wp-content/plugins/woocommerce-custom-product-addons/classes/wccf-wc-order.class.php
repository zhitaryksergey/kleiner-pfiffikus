...
class WCCF_WC_Order
{
    ...
    public function __construct()
    {
        ...
        add_filter('woocommerce_get_order_address', array($this, 'add_custom_address_field_values'), 99, 3);
    ...
    public function print_meta_box_order_fields($post)
    {
        ...
        $fields = WCCF_Order_Field_Controller::get_filtered(null, array('item_id' => $post->ID));
    ...
    public function save_backend_editing_field_values($post_id, $post)
    {
        ...
        foreach ($_POST['wccf_backend_editing'] as $item_id => $field_values) {
            ...
            foreach ($field_values as $field_id => $field_value) {
                ...
                // Store new value
                $value_to_store = apply_filters('wccf_field_value_to_store', $value, $item, $field);

                if ($field->context_is('user_field')) {
                    RightPress_WC::order_update_meta_data($item_id, $field->get_value_access_key(), $value_to_store);
                }
                else {
                    $field->update_data($item, $field->get_value_access_key($quantity_index), $value_to_store);
                }

                // Check if extra data needs to be updated
                if ($field->has_options() || $field->field_type_is(array('date', 'time', 'datetime'))) {
                    ...
                    // Update ISO date/time value if needed
                    if ($field->field_type_is(array('date', 'time', 'datetime'))) {

                        // Value is not empty - convert
                        if (!empty($value)) {

                            $extra_data[('iso_' . $field->get_field_type())] = $field->convert_datetime_value_format_to_iso($value);
                        }
                        // Value is empty - remove ISO entry
                        else if (isset($extra_data[('iso_' . $field->get_field_type())])) {

                            unset($extra_data[('iso_' . $field->get_field_type())]);
                        }
                    }
     ...
     * @access public
     * @param array $address
     * @param array $address_type
     * @param object $order
     * @return array
     */
    public function add_custom_address_field_values($address, $address_type, $order)
    {

        // Get WCCF_WC_User instance
        $wc_user = WCCF_WC_User::get_instance();

        // Add custom address field values for display
        $address = $wc_user->add_custom_address_field_values($address, $order, ($address_type . '_address'));
