...
class pt_popular_posts_widget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			...
			__('Handy Popular Posts', 'handy-feature-pack'),
			array( 'description' => __( 'Handy special widget. Outputs a list of the posts with the most user likes', 'handy-feature-pack' ), )
		);
	}
	...
	?>
		...
		<p>
            <label for="<?php echo $this->get_field_id("range"); ?>"><?php _e('Time Range:', 'handy-feature-pack'); ?></label>
        	<select class="widefat" id="<?php echo $this->get_field_id("range"); ?>" name="<?php echo $this->get_field_name("range"); ?>">
          		<option value="date" <?php selected( $instance["range"], "day" ); ?>><?php _e('Today', 'handy-feature-pack'); ?></option>
           		<option value="comment_count" <?php selected( $instance["range"], "week" ); ?>><?php _e('Week', 'handy-feature-pack'); ?></option>
         		<option value="title" <?php selected( $instance["range"], "month" ); ?>><?php _e('Month', 'handy-feature-pack'); ?></option>
				<option value="author" <?php selected( $instance["range"], "all" ); ?>><?php _e('All Time', 'handy-feature-pack'); ?></option>
        	</select>
        </p>
        <p>
	...
	public function widget( $args, $instance ) {
		...
		switch ($range) {
			case 'day':
				$today = date('j');
				$inner_title = __( 'Today\'s Most Popular Posts', 'handy-feature-pack' );
			break;
			case 'week':
				$week = date('W');
				$inner_title = __( 'This Month\'s Most Popular Posts', 'handy-feature-pack' );
			break;
			case 'month':
				$month = date('m');
				$inner_title = __( 'This Month\'s Most Popular Posts', 'handy-feature-pack' );
		...
		if ( $pop_posts->have_posts() ) {
			...
			$like_list .= "<h3>" . __( 'Nothing yet', 'handy-feature-pack' ) . "</h3>\n";
			$like_list .= "</div>\n";
			echo $like_list;
		}
		wp_reset_postdata();
