...
class pt_vendor_products_widget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			...
			__('Handy Vendor Products', 'handy-feature-pack'), // Name
			array('description' => __( "Handy special widget. Outputs list of products based on current vendor page.", 'handy-feature-pack' ), )
		);
	}
	...
	?>
		...
		<p>
            ...
            <label for="<?php echo $this->get_field_id('show-vendor-title'); ?>"><?php _e( 'Add Vendor Shop name to the title?', 'handy-feature-pack' ); ?></label>
        </p>
		<p>
			<label for="<?php echo $this->get_field_id('products-qty'); ?>"><?php _e( 'How many products to display: ', 'handy-feature-pack' ) ?></label>
        ...
        <p>
            <label for="<?php echo $this->get_field_id("show"); ?>"><?php _e('Show:', 'handy-feature-pack'); ?></label>
        	<select class="widefat" id="<?php echo $this->get_field_id("show"); ?>" name="<?php echo $this->get_field_name("show"); ?>">
          		<option value="featured" <?php selected( $instance["show"], "featured" ); ?>><?php _e( 'Featured Products', 'handy-feature-pack' ) ?></option>
          		<option value="recent" <?php selected( $instance["show"], "recent" ); ?>><?php _e( 'Recent Products', 'handy-feature-pack' ) ?></option>
          		<option value="sale" <?php selected( $instance["show"], "sale" ); ?>><?php _e( 'On-Sale Products', 'handy-feature-pack' ) ?></option>
				<option value="best_selling" <?php selected( $instance["show"], "best_selling" ); ?>><?php _e( 'Best Selling Products', 'handy-feature-pack' ) ?></option>
        	</select>
        </p>
	<?php
	}
		...
		if ( $vendor_id && $the_query->have_posts() ) {
			...
			if ($title) {
				...
				if ( $instance['show-vendor-title'] == true ) {
					echo __(' by ', 'handy-feature-pack') . $shop_name;
				}
				echo $after_title;
			}
