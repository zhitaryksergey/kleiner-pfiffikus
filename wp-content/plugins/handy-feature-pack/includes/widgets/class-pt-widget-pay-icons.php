...
class pt_pay_icons_widget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
	 		'pt_pay_icons_widget',
			__('Handy Payment Icons', 'handy-feature-pack'), // Name
			array( 'description' => __( 'Handy special widget. Add payment methods icons', 'handy-feature-pack' ), ),
			array( 'width' => 600, )
		);
	}
	...
	<div class="tz-widget-wrapper">
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e('Title:', 'handy-feature-pack'); ?></label>
			<input class="widefat" type="text" id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" value="<?php echo esc_attr( $instance['title'] ); ?>" />
		</p>

		<p class="option_wrap">
			<label for="<?php echo esc_attr($this->get_field_id('icon_size')); ?>"><?php esc_html_e('Icon Size:', 'handy-feature-pack'); ?></label>
		...
		<p class="option_wrap right">
			...
			<label for="<?php echo esc_attr($this->get_field_id('layout_type')); ?>"><?php esc_html_e('Inline Mode', 'handy-feature-pack'); ?></label>
		</p>

		<p class="clear-float">
			<label for="<?php echo esc_attr( $this->get_field_id ('precontent') ); ?>"><?php esc_html_e('Pre-Content', 'handy-feature-pack'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id ('postcontent') ); ?>"><?php esc_html_e('Post-Content', 'handy-feature-pack'); ?></label>
							...
							foreach ($payment_icons as $key => $value) {
								...
								<li>
									...
									<p class="option_wrap">
										...
										<label for="<?php echo esc_attr($this->get_field_name('payment_icons')); ?>[<?php echo esc_attr($key); ?>][active]"><?php esc_html_e('Show on frontend', 'handy-feature-pack'); ?></label>
									</p>

									<p>
										<label for="<?php echo esc_attr($this->get_field_name('payment_icons')); ?>[<?php echo esc_attr($key); ?>][label]"><?php esc_html_e('Label:', 'handy-feature-pack'); ?></label>
	...
	public function widget( $args, $instance ) {
		...
		if ( ! empty( $precontent ) ) echo '<div class="precontent">'.$precontent.'</div>';
		...
		foreach($payment_icons as $payment) :
			if ( array_key_exists('active', $payment) && $payment['active']=='yes' ) {
				...
				$img_src =  HANDY_FEATURE_PACK_URL . '/public/img/payment_icons/'.$img;
		...
		if ( ! empty( $postcontent ) ) echo '<div class="postcontent">'.$postcontent.'</div>';
