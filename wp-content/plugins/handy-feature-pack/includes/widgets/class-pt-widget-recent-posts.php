...
class pt_recent_post_widget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			...
			esc_html__('Handy Recent Posts/Products', 'handy-feature-pack'),
			array('description' => esc_html__( "Handy special widget. Displaying number of recent posts (or products) on your site.", 'handy-feature-pack' ), )
		);
	}
	...
	?>
		<p>
			<label for="<?php echo $this->get_field_id('title'); ?>"><?php esc_html_e( 'Title: ', 'handy-feature-pack' ) ?></label>
			<input class="widefat" id="<?php echo esc_attr( $this->get_field_id('title') ); ?>" name="<?php echo esc_attr( $this->get_field_name('title') ); ?>" type="text" value="<?php echo esc_attr( $instance['title'] ); ?>" />
		</p>
		<p>
			<label for="<?php echo $this->get_field_id('post-quantity'); ?>"><?php esc_html_e( 'How many posts to display: ', 'handy-feature-pack' ) ?></label>
		...
		<p>
      <label for="<?php echo $this->get_field_id('post-type'); ?>"><?php esc_html_e('Show Posts for next Post Type:', 'handy-feature-pack');?></label>
      <select class="widefat" id="<?php echo $this->get_field_id('post-type'); ?>" name="<?php echo $this->get_field_name('post-type'); ?>">
        <option value="post" <?php selected( $instance["post-type"], "post" ); ?>><?php esc_html_e( 'Posts', 'handy-feature-pack' ) ?></option>
        <option value="product" <?php selected( $instance["post-type"], "product" ); ?>><?php esc_html_e( 'Products', 'handy-feature-pack' ) ?></option>
      </select>
    </p>
    <p>
      <label for="<?php echo $this->get_field_id("sort-by"); ?>"><?php esc_html_e('Sort by:', 'handy-feature-pack'); ?></label>
      <select class="widefat" id="<?php echo $this->get_field_id("sort-by"); ?>" name="<?php echo $this->get_field_name("sort-by"); ?>">
        <option value="date" <?php selected( $instance["sort-by"], "date" ); ?>><?php esc_html_e( 'Date', 'handy-feature-pack' ) ?></option>
        <option value="title" <?php selected( $instance["sort-by"], "title" ); ?>><?php esc_html_e( 'Title', 'handy-feature-pack' ) ?></option>
        <option value="comment_count" <?php selected( $instance["sort-by"], "comment_count" ); ?>><?php esc_html_e( 'Number of comments', 'handy-feature-pack' ) ?></option>
				<option value="author" <?php selected( $instance["sort-by"], "author" ); ?>><?php esc_html_e( 'Author', 'handy-feature-pack' ) ?></option>
        <option value="rand" <?php selected( $instance["sort-by"], "rand" ); ?>><?php esc_html_e( 'Random', 'handy-feature-pack' ) ?></option>
      </select>
    </p>
    <p>
      ...
      <label for="<?php echo $this->get_field_id("sort-order"); ?>"><?php esc_html_e( 'Reverse sort order (ascending)?', 'handy-feature-pack' ); ?></label>
    </p>
    <p>
      <input type="checkbox" class="checkbox" id="<?php echo $this->get_field_id("date"); ?>" name="<?php echo $this->get_field_name("date"); ?>"<?php checked( (bool) $instance["date"] ); ?> />
    	<label for="<?php echo $this->get_field_id("date"); ?>"><?php esc_html_e( 'Show publish date?', 'handy-feature-pack' ); ?></label>
    </p>
    <p>
      ...
      <label for="<?php echo $this->get_field_id("cats"); ?>"><?php esc_html_e( 'Show list of post/products Categories?', 'handy-feature-pack' ); ?></label>
    </p>
    <p>
      ...
      <label for="<?php echo $this->get_field_id("comments"); ?>"><?php esc_html_e( 'Show number of comments/reviews?', 'handy-feature-pack' ); ?></label>
    </p>
		<p>
			<label for="<?php echo $this->get_field_id('category'); ?>"><?php esc_html_e( 'Specify ID of category (categories) to show: ', 'handy-feature-pack' ) ?></label>
    ...
    <p>
      ...
      <label for="<?php echo $this->get_field_id("thumb"); ?>"><?php esc_html_e( 'Show post/product featured image?', 'handy-feature-pack' ); ?></label>
    </p>
    <p>
      ...
      <label for="<?php echo $this->get_field_id("excerpt"); ?>"><?php esc_html_e( 'Show post/product excerpt?', 'handy-feature-pack' ); ?></label>
    </p>
    <p>
      <label for="<?php echo $this->get_field_id("excerpt-more"); ?>"><?php esc_html_e( 'Excerpt read more text:', 'handy-feature-pack' ); ?></label>
    ...
    <p>
      ...
      <label for="<?php echo $this->get_field_id("price"); ?>"><?php esc_html_e( 'Show price? (products only)', 'handy-feature-pack' ); ?></label>
    </p>
		<p>
			...
			<label for="<?php echo $this->get_field_id("show_in_menu"); ?>"><?php esc_html_e( 'Check if you want to add widget to Max Mega Menu', 'handy-feature-pack' ); ?></label>
		</p>
		<p>
      <label for="<?php echo $this->get_field_id("cols-qty"); ?>"><?php esc_html_e('Columns quantity for posts/products in Max Mega Menu:', 'handy-feature-pack'); ?></label>
      <select class="widefat" id="<?php echo $this->get_field_id("cols-qty"); ?>" name="<?php echo $this->get_field_name("cols-qty"); ?>">
        <option value="2" <?php selected( $instance["cols-qty"], "2" ); ?>><?php esc_html_e( '2 Columns', 'handy-feature-pack' ) ?></option>
        <option value="3" <?php selected( $instance["cols-qty"], "3" ); ?>><?php esc_html_e( '3 Columns', 'handy-feature-pack' ) ?></option>
        <option value="4" <?php selected( $instance["cols-qty"], "4" ); ?>><?php esc_html_e( '4 Columns', 'handy-feature-pack' ) ?></option>
        <option value="6" <?php selected( $instance["cols-qty"], "6" ); ?>><?php esc_html_e( '6 Columns', 'handy-feature-pack' ) ?></option>
      </select>
    </p>
	<?php
	}
    ...
    $query_args = array (
      'ignore_sticky_posts' => 1,
		...
		if ( $show_in_menu ) {
			...
			switch ($cols_qty) {
				case '2':
					$extra_class = ' col-xs-6 col-sm-4 col-md-6';
				break;
				case '3':
					$extra_class = ' col-xs-6 col-sm-4 col-md-4';
				break;
				case '4':
					$extra_class = ' col-xs-6 col-sm-4 col-md-3';
				break;
				case '6':
					$extra_class = ' col-xs-6 col-sm-4 col-md-2';
				break;
			}
		}
		...
		while ( $the_query->have_posts() ) :
			...
			<li class="recent-post-item<?php echo esc_attr($extra_class); ?>">
				...
				<div class="thumb-wrapper">
					<a class="recent-posts-img-link" rel="bookmark" href="<?php esc_url(the_permalink()); ?>" title="<?php esc_html_e('View details...', 'handy-feature-pack'); ?>">
				...
				<div class="content-wrapper">
					<h4><a href="<?php esc_url(the_permalink()); ?>" class="nav-button" rel="bookmark" title="<?php esc_html_e('View details...', 'handy-feature-pack'); ?>"><?php esc_attr(the_title()); ?></a></h4>
					...
					<?php if ($show_date || $show_comments || $show_cats) { ?>
						...
						if ($show_cats && $post_type != 'product') {
							$categories_list = get_the_category_list( __( ', ', 'handy-feature-pack' ) );
							...
							if ( ! empty( $categories ) ) {
							    ...
							    foreach ( $categories as $categorie ) {
							        $i++;
							        $term_list .= '<a href="' . esc_url( get_term_link( $categorie ) ) . '" title="' . esc_attr( sprintf( esc_html__( 'View all post filed under %s', 'handy-feature-pack' ), $categorie->name ) ) . '">' . $categorie->name . '</a>';
							        if ( $count != $i ) {
							            $term_list .= ' &middot; ';
							        }
							        else {
					...
					<div class="recent-posts-entry-content"><?php the_excerpt(); ?>
						<a href="<?php esc_url(the_permalink()); ?>" class="more-link" rel="bookmark" title="<?php esc_html_e('View details...', 'handy-feature-pack'); ?>"><?php echo esc_attr($excerpt_more); ?></a>
					</div>
