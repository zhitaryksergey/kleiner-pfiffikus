...
class WC_GZDP_Invoice_Cancellation extends WC_GZDP_Invoice {
	...
	public function get_order_number() {
		return ( $this->parent ) ? $this->parent->get_order_number() : '';
	}

    public function negate_string( $val ) {
        return ( substr( strip_tags( $val ), 0, 1 ) === '-' ? str_replace( '-', '', $val ) : '-' . $val );
    }

    public function get_totals( $tax_display = 'incl', $is_refund = false ) {
        ...
        // Remove refund and subtotal from cancellations
        foreach( $totals as $key => $total ) {
            if ( strpos( $total['key'], 'refund_' ) !== false || 'cart_subtotal' === $total['key'] ) {
                unset( $totals[ $key ] );
            }
        }
	...
	public function get_order() {
		return $this->parent ? $this->parent->get_order() : false;
	}
