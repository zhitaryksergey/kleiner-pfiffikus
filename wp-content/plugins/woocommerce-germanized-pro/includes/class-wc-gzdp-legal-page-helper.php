...
class WC_GZDP_Legal_Page_Helper {
	...
	public function remove_plain_attachments( $do_attach, $mail, $page_key ) {
		...
		if ( isset( $pages[ $page_key ] ) ) {
			...
			$id = $pages[ $page_key ];
			$legal = new WC_GZDP_Legal_Page( $id, $page_key );
	...
	public function attach_pdfs( $attachments, $mail_id, $mail ) {
		...
		foreach ( $this->get_legal_page_ids() as $page => $id ) {
			...
			$legal     = new WC_GZDP_Legal_Page( $id, $page );
			$templates = array();
	...
	public function download_legal_page() {
		if ( isset( $_GET[ 'action' ] ) && $_GET[ 'action' ] == 'wc-gzdp-download-legal-page' ) {
			...
			if ( isset( $_GET[ 'preview' ] ) && wp_verify_nonce( $_REQUEST['_wpnonce'], 'wc-gzdp-download' ) ) {
				...
				$id = absint( $_GET[ 'id' ] );
				$page = ( isset( $_GET[ 'slug' ] ) ? sanitize_text_field( $_GET[ 'slug' ] ) : '' );
				$page = new WC_GZDP_Legal_Page( $id, $page );
				$page->refresh();
	...
	public function pdf_attachment_settings() {
		$mails    = array();
		...
		foreach ( $pages as $pagename => $id ) {
			$plain_id = $pagename;
			$page_id  = '';

			if ( ! $page_id = wc_get_page_id( $pagename ) ) {
				continue;
			}
			...
			$legal_page = new WC_GZDP_Legal_Page( $page_id, $pagename );
			$settings   = array_merge( $settings, array(

				array(
					'title' 	=> $this->get_legal_page_title( $pagename ),
					'desc' 		=> ( $legal_page->get_pdf_path() ? sprintf( _x( 'Send %s as <a href="%s" target="_blank">PDF Attachment</a> instead of plain text.', 'legal-page', 'woocommerce-germanized-pro' ), $this->get_legal_page_title( $pagename ), $legal_page->get_pdf_url() ) : _x( 'Send as PDF Attachment instead of plain text.', 'legal-page', 'woocommerce-germanized-pro' ) ),
				...
				array(
					...
					'desc' 		=> '<div class="wc-gzd-additional-desc">' . sprintf( _x( 'Manually choose PDF file to attach as %s. Leave empty to automatically generate PDF files from your pages\' text. Edit layout options <a href="%s">here</a>.', 'legal-page', 'woocommerce-germanized-pro' ), $this->get_legal_page_title( $pagename ), admin_url( 'admin.php?page=wc-settings&tab=germanized-emails&section=attachments_pdf' ) ) . '</div>',
					'id' 		=> 'woocommerce_gzdp_legal_page_' . $plain_id . '_pdf',
					'default'	=> '',
					'custom_attributes' => array(
						'data-show_if_woocommerce_gzdp_legal_page_' . $plain_id . '_enabled' => '',
					),
