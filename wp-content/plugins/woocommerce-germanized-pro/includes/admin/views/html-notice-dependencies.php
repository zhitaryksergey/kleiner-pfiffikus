...
<div id="message" class="error woocommerce-gzd-message wc-connect">
	...
	<?php foreach ( $dependencies->plugins_result as $type => $plugins ) : ?>
		...
		<?php if ( $type === 'unactivated' && ! empty( $plugins ) ) : ?>
			...
			<p><?php _e( 'To use Germanized Pro you may at first install the following plugins:', 'woocommerce-germanized-pro' ); ?></p>
		...
		<?php elseif ( $type === 'outdated' && ! empty( $plugins ) ) : ?>
			...
			<p><?php _e( 'To use Germanized Pro you may at first update the following plugins to a newer version:', 'woocommerce-germanized-pro' ); ?></p>
