...
class WC_GZDP_Post_PDF {
	...
	protected function refresh_pdf() {
	...
	public function get_pdf_template( $first = false ) {
		...
		return apply_filters( 'woocommerce_gzdp_get_pdf_template', $return, $first, $this );
	}
	...
	public function generate_pdf( $preview = false ) {
		...
		$filename_exists = $this->filename_exists();
		$upload_dir      = WC_germanized_pro()->get_upload_dir();
		$filename        = $this->get_filename();
		$path            = trailingslashit( $upload_dir['path'] ) . $filename;
		$file            = $pdf->Output( $path, ( $preview ) ? 'I' : 'S' );

		if ( ! $preview ) {
			if ( $tmp_path = wc_gzdp_upload_file( $filename, $file, $filename_exists ) ) {
				$path = $tmp_path;
			}
        }
