jQuery( function ( $ ) {
	...
	$( '.wc-gzdp-generator .form-table tr input, .wc-gzdp-generator .form-table tr select' ).bind( 'check_input', function () {
		...
		$( '.wc-gzdp-generator' ).find( 'input[data-show_if_' + name + '], select[data-show_if_' + name + '], textarea[data-show_if_' + name + ']' ).each( function () {
	...
	$( '.wc-gzdp-generator input, .wc-gzdp-generator select, .wc-gzdp-generator textarea' ).bind( 'check_show_if_checked', function ( e, elem ) {
		var i_val = $( elem ).val();
	...
	$( '.wc-gzdp-generator input, .wc-gzdp-generator select, .wc-gzdp-generator textarea' ).bind( 'check_show_if_unchecked', function ( e, elem ) {
		var i_val = $( elem ).val();
		var name = $( elem ).attr( 'name' ).replace( '[]', '' );
		var cur_val = $( this ).data( 'show_if_' + name );
		var hide = true;
	...
	$( '.wc-gzdp-generator .form-table tr' ).each( function () {
		var $mandatory = $( this ).find( ':input[data-mandatory^="<span"]' );
		$mandatory.addClass( 'wc-gzdp-input-mandatory' );
		if ( $mandatory.length > 0 ) {
			var mandatory_str = $mandatory.data( 'mandatory' );
			$( this ).find( 'label:first' ).append( ' ' + mandatory_str );
		}
	} );
...
*/
	...
	if ( $( '.wc-gzdp-generator' ).length > 0 ) {
		...
		$( document ).on( 'submit', '.woocommerce #mainform', function () {
			...
			var form_is_submittable = false;

			$( '.wc-gzdp-generator' ).find( ':input.wc-gzdp-input-mandatory' ).each( function () {
				var $input = $( this );
				var is_valid = true;

				$input.removeClass( 'wc-gzdp-input-error' );
				...
				if ( !is_valid ) {
					form_is_submittable = false;
					$input.addClass( 'wc-gzdp-input-error' );
				}
			} );

			if ( !form_is_submittable ) {

				$( [document.documentElement, document.body] ).animate( {
					scrollTop: ($( '.wc-gzdp-input-error:first' ).offset().top - 60)
				}, 1000 );
	...
	$( document ).on( 'click', '.button-wc-gzdp-attachment-field', function ( e ) {
		...
		custom_file_frame.on( 'select', function () {
			...
			var cur_attachments = string.split( ',' );

			if ( !allow_multiple && cur_attachments.length > 0 ) {
				$.each( cur_attachments, function ( i, val ) {
					var remove = true;
					...
					if ( remove ) {
						wrapper.find( 'a.unset-attachment[data-unset="' + val + '"]' ).trigger( 'click' );
					}
				} );
			...
			$.each( attachments, function ( index, attachment ) {
				if ( $.inArray( String( attachment.id ), currents ) === - 1 ) {
					string += ((index > 0 || string.length > 0) ? "," : "") + attachment.id;

					wrapper.find( '.current-attachment' ).append(
						'<div class="wc-gzdp-unset-attachment-wrap data-unset-' + attachment.id + '"><a class="unset-attachment" data-unset="' + attachment.id + '" href="#"><span class="dashicons dashicons-no-alt"></span></a> <a href="' + attachment.editLink + '" target="_blank">' + attachment.title + '</a></div>'
					);
				}
			} );
	...
	$( '.wc-gzdp-input-reset-locked' ).each( function () {

		var propName = $( this ).attr( 'name' );

		$( this ).after( '<span class="wc-gzdp-reset-checkbox"><input type="checkbox" class="' + propName + '_lock" name="' + propName + '_lock" value="1" /> ' + $( this ).data( 'unlock-title' ) + '</span>' );
		$( this ).prop( 'readonly', 'readonly' );

		$( document ).on( 'change', '.' + propName + '_lock', function () {
			if ( $( this ).is( ':checked' ) ) {
				$( this ).parents( 'td' ).find( 'input.wc-gzdp-input-reset-locked' ).prop( 'readonly', '' );
			} else {
				$( this ).parents( 'td' ).find( 'input.wc-gzdp-input-reset-locked' ).prop( 'readonly', 'readonly' );
			}
		} );
	} );

	$( document ).on( 'change', '#woocommerce_gzdp_vat_requester_vat_id', function() {
		var $input 		= $( this ).parents( 'table' ).find( '#woocommerce_gzdp_vat_id_additional_field_check' ),
		...
		if ( $( this ).val().length > 0 ) {
			...
			$( document.body ).trigger( 'woocommerce_gzd_setting_field_visible', [ $field, currentName, $input.val() ] );
		} else {
			$field.addClass( 'wc-gzd-setting-invisible' );
			$( document.body ).trigger( 'woocommerce_gzd_setting_field_invisible', [ $field, currentName, $input.val() ] );
		}
	} );

	if ( $( '#woocommerce_gzdp_vat_requester_vat_id' ).length > 0 ) {
		$( '#woocommerce_gzdp_vat_requester_vat_id' ).trigger( 'change' );
	}
} );